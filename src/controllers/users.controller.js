...
const togglePremiumFeature = async (req, res, next) => {
    try {
        ...
        res.send({ status: 1, msg: `Premium feature for user ${user.email} successfully toggled. New role is ${user.role}` });
    } catch (error) {
        next(error);
    }
};
...
const updateDocuments = async (req, res, next) => {
    try {
        ...
        res.send({ status: 1, msg: `Documents for user ${user.email} successfully updated.`, userDocuments: user.documents });
    } catch (error) {
        next(error);
    }
}
