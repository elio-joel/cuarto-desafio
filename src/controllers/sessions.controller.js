...
const login = (req, res) => {
      res.cookie(process.env.AUTH_COOKIE, req.user, { httpOnly: true }).send({ status: 1, msg: 'User successfully logged in', jwt: req.user });
};
...
const logout = (req, res) => {
    ...
    res.clearCookie(process.env.AUTH_COOKIE).send({ status: 1, msg: 'User successfully logged out' });
};
