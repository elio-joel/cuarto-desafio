...
import { testUser, newUser, testUserPassword } from './data-sc.js';
...
describe('Test api/sessions endpoints', () => {
    ...
    describe('register', () => {
        ...
        it('POST Should register valid user', async () => {
            ...
            expect(body.msg).to.equal('New User registered');
        ...
        it('POST Should not register user with existing email', async () => {
            ...
            expect(body.msg).to.equal('User already exists');
        });
    ...
    describe('login', () => {
        ...
        it('POST Should login valid user', async () => {
            ...
            expect(statusCode).to.equal(200);
            expect(body.status).to.equal(1);
            expect(body.msg).to.equal('User successfully logged in');
            ...
            expect(cookie.name).to.equal('userjwt');
        ...
        it('POST Should not login invalid user', async () => {
            ...
            expect(statusCode).to.equal(401);
            expect(body.status).to.equal(0);
            expect(body.msg).to.equal('Wrong User');
        });
    ...
    describe('currentuser', () => {
            ...
            it('GET Should get current user', async () => {
                const { statusCode, ok, body } = await requester.get('/api/sessions/currentuser').set('Cookie', [`userjwt=${session}`]);
                expect(ok).to.equal(true);
                expect(statusCode).to.equal(200);
                expect(body.status).to.equal(1);
                expect(body.msg).to.equal('User logged in');
                expect(body.user).to.be.a('object');
        ...
        describe('logout', () => {
            ...
            it('POST Should logout', async () => {
                const { statusCode, ok, body } = await requester.post('/api/sessions/logout').set('Cookie', [`userjwt=${session}`]);
                expect(ok).to.equal(true);
                expect(statusCode).to.equal(200);
                expect(body.status).to.equal(1);
                expect(body.msg).to.equal('User successfully logged out');
            });
